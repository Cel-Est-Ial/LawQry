...
def add_radio_button(root, variable, value, text, relx, rely, relwidth=None, relheight=None, command=None, bg_color="#25282b", fg_color="white"):
    """Adds a custom radio button to the Tkinter window with customizable size and position."""
    radio_button = tk.Radiobutton(root, text=text, variable=variable, value=value,
                                   command=command, indicatoron=0, relief="raised", 
                                   bg=bg_color, fg=fg_color, activebackground="green")
    
    # Place the radio button with optional width and height
    radio_button.place(relx=relx, rely=rely, relwidth=relwidth, relheight=relheight, anchor="w")  
    return radio_button


def add_standard_button(root, text, relx, rely, command=None, bg_color="#FFFFFF", fg_color="#000000", relwidth=None, relheight=None):
    """Adds a standard button to the Tkinter window with relative width and height."""
